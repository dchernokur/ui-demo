      ...
      <Link
        className={`text-xs uppercase py-2 font-bold block ${
          usePathname().endsWith(link.url)
            ...
            : link.title === "+ Building" ? "text-red-600 hover:text-red-700" : "text-slate-700 hover:text-slate-500"
          }`}
        href={link.url}
        key={link.url}
      >
      ...
      <div className="drawer-content flex flex-col">
        <div className="navbar bg-base-300 w-full">
          <div className="flex-none lg:hidden">
            <label htmlFor="drawer-toggler" aria-label="open sidebar" className="btn btn-square btn-ghost">
              {FENavbarTogglerIcon}
            </label>
          </div>
          <div className="mx-2 flex-1 px-2" />
            <div className="hidden flex-none lg:block">
              <ul className="menu menu-horizontal">
                {links[linksForPage].filter(linkEntry => !linkEntry.hideFromNavbar).map(linkEntry => renderNavbarItem(linkEntry))}
              </ul>
              <ul className="menu menu-horizontal">
        ...
        <div className="drawer-side">
          <label htmlFor="drawer-toggler" aria-label="close sidebar" className="drawer-overlay" />
          <ul className="menu bg-base-200 min-h-full w-80 py-8 px-4">
            {links[linksForPage].map(linkEntry => renderNavbarItem(linkEntry, true))}
          </ul>
        </div>
    </div>
  )
